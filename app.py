            ...
            with st.spinner("AI 正在深度解析简历与岗位的契合度..."):
                ...
                try:
                    response = client.chat.completions.create(
                        ...
                        # 注意：如果 prompt 要求返回数组 []，有些模型不支持 json_object 模式
                        # 我们这里去掉 response_format 限制，或者确保 prompt 要求返回对象
                    )
...
if st.button("🪄 启动专家级精修"):
    if not final_sections or not target_jd:
        ...
        with st.status("🚀 专家正在深度重构中...", expanded=True) as status:
            # 合并当前所有输入内容作为 AI 的背景参考
            full_context_text = "\n".join([f"【{k}】: {v}" for k, v in final_sections.items()])
            ...
            for section_name, section_content in final_sections.items():
                if not section_content.strip(): continue  # 跳过空模块

                status.write(f"正在重构：{section_name}...")

                # 这里使用你原来的 specific_prompt 逻辑，但内容替换为 final_sections 中的内容
                # 注意：Prompt 里的内容来源要从 section_content 获取，而不是 cv_content
                # --- 在 for section_name, section_content in final_sections.items(): 循环内部 ---
                ...
                refined_data[section_name] = response.choices[0].message.content
            ...
            try:
                ...
                final_summary = "全局总结生成失败，请查看下方明细。"
...
if chat_input := st.chat_input("例如：'帮我把这段经历写得更有领导力一点'"):
    ...
    with st.chat_message("assistant"):
        response = client.chat.completions.create(
            model="deepseek-chat",
            messages=[{"role": "system", "content": "你是一个资深简历优化专家。"}] + st.session_state.messages
        )
